...
function MinimapContent({ floor, highlightColorMap }: SeatMinimapProps) {
  ...
  return createPortal(
    <div
      className="border-accent bg-muted-foreground fixed right-4 bottom-4 z-50 cursor-pointer overflow-hidden rounded-lg border shadow-lg backdrop-blur-sm select-none"
    ...
    >
      ...
      <div className="absolute inset-0 flex flex-col gap-1 p-1.5">
        {floor.rows.map((floorRow) => (
          <div key={floorRow.id} className="flex min-h-0 flex-1 gap-1">
            {floorRow.items.map((item) =>
              item.kind === 'aisle' ? (
                ...
                <div
                  ...
                  className={cn(
                    'bg-accent relative min-w-0 flex-1 rounded-sm',
                    sectionHasHighlight(item) && 'ring-primary ring-1',
                  )}
                >
                  {sectionHasHighlight(item) && (
                    <span className="bg-danger absolute top-0.5 left-0.5 h-1.5 w-1.5 rounded-full" />
                  )}
                </div>
              ),
            )}
      ...
      {minimapScale > 0 && (
        <div
          className="pointer-events-none absolute rounded-xs border border-white/60 bg-white/10"
          style={{
            left: viewportX,
            top: viewportY,
